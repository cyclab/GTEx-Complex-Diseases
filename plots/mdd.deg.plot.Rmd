...
ggplot(data=long, aes(x=tissue, y=value, fill=variable)) +
  ...
  scale_y_continuous(limits=c(0, 60), expand = c(0.02, 0)) +
  #geom_text(aes(label=count), vjust=0.25, hjust=-0.3, size=2.7) +
  labs(title="", x="", y="", fill="") +  scale_fill_manual(labels = c("up-regulated", "down-regulated"),values=c('#5fa778','#cae2d0')) +
...
v.all <- venn(list(`Brain - Hypothalamus`=deg.list[['Brain - Hypothalamus']]$total$ID,
                   ...
                   `Whole Blood`=deg.list[['Whole Blood']]$total$ID,
