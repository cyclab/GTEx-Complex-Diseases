...
sc_df <- data.frame(type = all_types, total = rep(0, length(all_types)), 
                    up = rep(0, length(all_types)), down = rep(0, length(all_types)))

for(i in all_types){
  tmp <- read.table(file.path(table_dir, paste0(i, ".tsv")), sep="\t", header=T)
  sc_df[sc_df$type==i,]$total <- nrow(tmp[tmp$adj.P.Val < 0.1 & abs(tmp$logFC) > log2(1.5),])
  sc_df[sc_df$type==i,]$up <- nrow(tmp[tmp$adj.P.Val < 0.1 & tmp$logFC > log2(1.5),])
  sc_df[sc_df$type==i,]$down <- nrow(tmp[tmp$adj.P.Val < 0.1 & tmp$logFC < -log2(1.5),])
}

sc_df <- sc_df[sc_df$total != 0,]

library(reshape2)
long <- melt(sc_df, id.vars = 'type')
long <- long[long$variable!='total',]


ggplot(data=long, aes(x=type, y=value, fill=variable)) +
...
cb_df <- read.delim(file.path(main_dir, "results/combined.brain.p.txt"), sep="\t", header=T, row.names = 1)
```

##### Heatmaps for three brain clusters
###### Use DEGs with combined Fisher's q-val < 0.05

```{R, fig.height=13, fig.width=16, eval=F}
cb_df <- read.delim(file.path(main_dir, "results/combined.brain.p.txt"), sep="\t", header=T, row.names = 1)
