...
#Extract columns
with open('/Users/magicpants/GoogleDrive/v6.data/new/limma.results/combined.brains.txt') as f:
    next(f) #pass header
    lines = f.readlines()

    for x in lines:
        genes.append(x.split("\t")[2])
        brain0t.append(float(x.split("\t")[3]))
        brain1t.append(float(x.split("\t")[5]))
        brain2t.append(float(x.split("\t")[7]))
        combq.append(-math.log10(float(x.split("\t")[9])))
...
plt.savefig('/Users/magicpants/GoogleDrive/v6.data/new/limma.results/3Dscatterplot.pdf',format = 'pdf')
